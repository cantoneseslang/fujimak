...
export default function HistoryPageClient() {
  ...
  const { data: requests = [], isPending: maintenancePending } = useMaintenanceRequestsQuery(
    { storeId: selectedStoreId ?? '', limit: 220 },
    Boolean(selectedStoreId)
  )
  const isLoading = Boolean(selectedStoreId) && maintenancePending
  ...
  useEffect(() => {
    ...
    const loadSupportThreads = async () => {
      ...
      try {
        const res = await fetch(`/api/support/threads?status=all&storeId=${encodeURIComponent(selectedStoreId)}`, {
          cache: 'no-store',
        })
